...
{
    public class UdpContact : Contact, IEquatable<UdpContact>
    {
        private byte[] addressBytes
        {
            get { return Extensible.GetValue<byte[]>(this, NEXT_PROTO_TAG); }
            set { Extensible.AppendValue(this, NEXT_PROTO_TAG, value); }
        }

        public IPAddress Ip
        {
            get { return new IPAddress(addressBytes); }
            private set { addressBytes = value.GetAddressBytes(); }
        }

        public int Port
        {
            get { return Extensible.GetValue<int>(this, NEXT_PROTO_TAG + 1); }
            private set { Extensible.AppendValue<int>(this, NEXT_PROTO_TAG + 1, value); }
        }

        public UdpContact(Identifier512 id, Guid networkId, IPAddress ip, int port) : base(id, networkId)
        {
            Ip = ip;
            Port = port;
        }
        ...
        public static void InitialiseUdp(DistributedRoutingTable localTable, int port)
        {
            ...
            listenThread = new Thread((  ) =>
            {
                while (listen)
                {
                    ...
                    while (!async.IsCompleted && listen)
                        Thread.Sleep(10);
