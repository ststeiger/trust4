...
namespace Daylight
{
    ...
    public class LocalStorage
    {
        ...
        private SortedList<ID, SortedList<ID, Entry>> store;
        ...
        /// </summary>
        public LocalStorage(IPEndPoint endpoint, ID id)
        {
        ...
        {
            saveThread.Abort();
            // Stop our autosave thread.
            SaveIndex();
            // Make sure our index getw written when we shut down properly.
            mutex.Close();
            // Release our hold on the mutex.
        }
            ...
            lock (store)
            {
                if (ContainsKey(key))
                {
                    foreach (ID hash in store[key].Keys)
                        ...
                        toReturn.Add(new Result(new Contact(this.m_SelfID, this.m_EndPoint), File.ReadAllText(PathFor(key, hash))));
                }
            }
            return toReturn;
        }
            ...
            lock (store)
            {
                for (int i = 0; i < store.Count; i++)
                {
                    ...
                    for (int j = 0; j < vals.Count; j++)
                        if (DateTime.Now.ToUniversalTime() > vals.Values[j].timestamp + vals.Values[j].keepFor)
                        {
                            // Too old!
                            // Delete file
