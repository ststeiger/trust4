...
namespace Daylight
{
    ...
    public class KademliaNode
    {
        ...
        private Thread bucketMinder;
        // Handle updates to cache
        private const int CHECK_INTERVAL = 1;
        private List<Contact> contactQueue;
        // Add contacts here to be considered for caching
        ...
        private TimeSpan MAX_SYNC_WAIT = new TimeSpan(5000000);
        // 500 ms in ticks
        // Application (datastore)
        private LocalStorage datastore;
        // Keep our key/value pairs
        private SortedList<ID, DateTime> acceptedStoreRequests;
        ...
        // Kademlia config
        private const int PARALELLISM = 3;
        // Number of requests to run at once for iterative operations.
        private const int NODES_TO_FIND = 20;
        // = k = bucket size
        private static TimeSpan EXPIRE_TIME = new TimeSpan(24, 0, 0);
        // Time since original publication to expire a value
        private static TimeSpan REFRESH_TIME = new TimeSpan(1, 0, 0);
        // Time since last bucket access to refresh a bucket
        private static TimeSpan REPLICATE_TIME = new TimeSpan(1, 0, 0);
        ...
        public IList<Result> Get(ID key)
        {
            if (datastore.ContainsKey(key))
            {
                ...
                Console.WriteLine("SEARCH INFORMATION: Found result data in the local data store.");
            ...
            else
            {
                ...
                if (found == null || found.Count == 0)
                {
                    ...
                    Console.WriteLine("SEARCH ERROR: No results found, but the nearest contacts are:");
                    foreach (Contact c in close)
                    {
                        Console.WriteLine("              * " + c.GetEndPoint().ToString());
                    }
                    return new List<Result>();
                }
                else
                {
                    Console.WriteLine("SEARCH INFORMATION: Found result data in the DHT.");
                    return found;
                }
            }
        }
        ...
        private IList<Contact> IterativeFind(ID target, bool getValue, out IList<Result> vals)
        {
            ...
            while (shortlistIndex < shortlist.Count && shortlistIndex < NODES_TO_FIND)
            {
                ...
                // TODO: WTF does the spec want with this
        ...
        private void HandleStoreQuery(Contact sender, StoreQuery request)
        {
            ...
            {
                acceptedStoreRequests[request.GetConversationID()] = DateTime.Now;
                // Record that we accepted it
                SendMessage(sender.GetEndPoint(), new StoreResponse(nodeID, request, true));
            }

            else if (request.GetPublicationTime() > datastore.GetPublicationTime(key, valHash) && request.GetPublicationTime() < DateTime.Now.ToUniversalTime().Add(MAX_CLOCK_SKEW))
        ...
        private void HandleStoreResponse(Contact sender, StoreResponse response)
        {
            lock (sentStoreRequests)
            {
                if (response.ShouldSendData() && sentStoreRequests.ContainsKey(response.GetConversationID()))
                {
                    ...
                    SendMessage(sender.GetEndPoint(), new StoreData(nodeID, response, toStore.key, ID.Hash(toStore.val), toStore.val, toStore.publication));
            ...
            while (true)
            {
                ...
                while (contactQueue.Count > 0)
                {
                    ...
                    if (contactCache.Contains(applicant.GetID()))
                    {
                        ...
                        if (contactCache.Get(applicant.GetID()).GetEndPoint() != applicant.GetEndPoint())
                        {
                            // Replace old one
